...
© 2026 闲鱼管理系统"""
            ...
            try:
                ...
                if user_id is not None:
                    ...
                    SELECT dr.id, dr.keyword, dr.item_id, dr.card_id, dr.delivery_count, dr.enabled,
                           ...
                           (SELECT ii.item_title FROM item_info ii WHERE ii.item_id = dr.item_id ORDER BY ii.created_at DESC LIMIT 1) as item_title
    ...
    def get_delivery_rules_by_item_id(self, item_id: str, user_id: int = None, only_non_multi_spec: bool = False):
        """根据商品ID获取匹配的发货规则"""
        if not item_id:
            return []
        with self.lock:
            try:
                ...
                cursor.execute(f"""
                    ...
                    FROM delivery_rules dr
                    LEFT JOIN cards c ON dr.card_id = c.id
                    WHERE dr.enabled = 1 AND c.enabled = 1 AND dr.item_id = ?
                    {user_filter}
                    {non_multi_filter}
                    ORDER BY dr.id ASC
                """, params)
                ...
                for row in cursor.fetchall():
                    rules.append({
                        'id': row[0], 'keyword': row[1], 'item_id': row[2], 'card_id': row[3],
                        'delivery_count': row[4], 'enabled': bool(row[5]), 'description': row[6],
                        'delivery_times': row[7], 'card_name': row[8], 'card_type': row[9],
                        'api_config': row[10], 'text_content': row[11], 'data_content': row[12],
                        'image_url': row[13], 'card_enabled': bool(row[14]), 'card_description': row[15],
                        'card_delay_seconds': row[16], 'is_multi_spec': bool(row[17]) if row[17] is not None else False,
                        'spec_name': row[18], 'spec_value': row[19], 'spec_name_2': row[20], 'spec_value_2': row[21]
                    })
                return rules
            except Exception as e:
        ...
        with self.lock:
            try:
                ...
                if expected_mode == 'two_spec':
                    ...
                    AND LOWER(REPLACE(REPLACE(TRIM(COALESCE(c.spec_name, '')), ' ', ''), '　', '')) = ?
                    AND LOWER(REPLACE(REPLACE(TRIM(COALESCE(c.spec_value, '')), ' ', ''), '　', '')) = ?
                    AND LOWER(REPLACE(REPLACE(TRIM(COALESCE(c.spec_name_2, '')), ' ', ''), '　', '')) = ?
                    AND LOWER(REPLACE(REPLACE(TRIM(COALESCE(c.spec_value_2, '')), ' ', ''), '　', '')) = ?
                    ORDER BY dr.id ASC
                    """
                    params = [item_id]
                    if user_id is not None:
                        params.append(user_id)
                    params.extend([normalized_spec_name, normalized_spec_value, normalized_spec_name_2, normalized_spec_value_2])
                else:
                    sql = f"""
                    SELECT dr.id, dr.keyword, dr.item_id, dr.card_id, dr.delivery_count, dr.enabled,
                           dr.description, dr.delivery_times,
                    ...
                    AND LOWER(REPLACE(REPLACE(TRIM(COALESCE(c.spec_name, '')), ' ', ''), '　', '')) = ?
                    AND LOWER(REPLACE(REPLACE(TRIM(COALESCE(c.spec_value, '')), ' ', ''), '　', '')) = ?
                    AND COALESCE(TRIM(c.spec_name_2), '') = ''
                    AND COALESCE(TRIM(c.spec_value_2), '') = ''
                    ORDER BY dr.id ASC
                    """
                    params = [item_id]
                    if user_id is not None:
                        params.append(user_id)
                    params.extend([normalized_spec_name, normalized_spec_value])
                cursor.execute(sql, params)
                rules = []
                for row in cursor.fetchall():
                    rules.append({
