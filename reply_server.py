...
def _get_announcement_remote_url() -> str:
    ...
    owner = str(os.getenv('UPDATE_GITHUB_OWNER') or 'RRRLLW').strip() or 'RRRLLW'
    repo = str(os.getenv('UPDATE_GITHUB_REPO') or 'xianyu_plus').strip() or 'xianyu_plus'
...
def create_delivery_rule(rule_data: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        rule_id = db_manager.create_delivery_rule(
            keyword=rule_data.get('keyword') or rule_data.get('item_title') or rule_data.get('item_id'),
