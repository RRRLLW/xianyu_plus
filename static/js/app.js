...
function renderDeliveryRulesList(rules) {
    ...
    tr.innerHTML = `
        <td>
        <div class="fw-bold">${escapeHtml(rule.item_title || rule.keyword || ("商品ID: " + (rule.item_id || "未知商品")))}</div><small class="text-muted d-block">商品ID: ${escapeHtml(rule.item_id || "-")}</small>
...
function showAddDeliveryRuleModal() {
    ...
    loadDeliveryItemsForSelect(); // 加载商品选项
...
// 加载商品列表用于发货规则选择
async function loadDeliveryItemsForSelect(selectedValue = '', editSelectedValue = '') {
    try {
    const response = await fetch(`${apiBase}/items`, {
        headers: {
        'Authorization': `Bearer ${authToken}`
    ...
    const items = data.items || [];
    const targets = [
        { el: document.getElementById('deliveryItemId'), value: selectedValue },
        { el: document.getElementById('editDeliveryItemId'), value: editSelectedValue }
    ];

    targets.forEach(({ el, value }) => {
        if (!el) return;
        el.innerHTML = '<option value="">请选择商品</option>';
        const seen = new Set();
        items.forEach(item => {
            if (!item || !item.item_id || seen.has(item.item_id)) return;
            seen.add(item.item_id);
            const option = document.createElement('option');
            option.value = item.item_id;
            option.textContent = `${item.item_title || '未知商品'} (${item.item_id})`;
        ...
        if (value) {
            ...
            if (el.value !== value) {
                ...
                fallback.textContent = `当前规则商品 (${value})`;
...
async function saveDeliveryRule() {
    ...
    const itemTitle = selectedOption ? selectedOption.textContent : '';
...
async function editDeliveryRule(ruleId) {
    ...
    if (response.ok) {
        ...
        await loadDeliveryItemsForSelect('', rule.item_id || '');
        const editItemSelect = document.getElementById('editDeliveryItemId');
        if (editItemSelect && rule.item_id) {
            editItemSelect.value = rule.item_id;
        }
...
async function updateDeliveryRule() {
    ...
    const itemSelect = document.getElementById('editDeliveryItemId');
    const itemId = itemSelect.value;
    const selectedOption = itemSelect.selectedOptions[0];
    const itemTitle = selectedOption ? selectedOption.textContent : '';
